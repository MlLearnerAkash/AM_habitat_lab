...
#                 "linear_friction", "angular_friction",
...
#-----------------------------Start of new code for habitat-0.3.3----------------
import magnum as mn
import numpy as np
...
import habitat_sim
# Corrected imports for 0.3.3
from habitat_sim.agent import AgentConfiguration, AgentState
from habitat_sim.sensor import SensorType, SensorSpec, CameraSensorSpec
...
def overwrite_config(
    ...
    for attr, value in config_from.items():
        ...
        if ignore_keys is None or low_attr not in ignore_keys:
            if hasattr(config_to, low_attr):
                # 0.3.3 is stricter with Magnum types
                if isinstance(value, (list, tuple)) and len(value) == 3:
                    value = mn.Vector3(value)
                try:
                    setattr(config_to, low_attr, value)
                except AttributeError:
                    # Catch read-only attributes common in 0.3.3 bindings
                    continue
...
class HabitatSim(habitat_sim.Simulator, Simulator):
    def __init__(self, config: Config) -> None:
        ...
        # 0.3.3 initialization logic
        self.sim_config = self.create_sim_config(self._sensor_suite)
        self._current_scene = self.sim_config.sim_cfg.scene_id
        
        # Call base habitat_sim.Simulator constructor
        super().__init__(self.sim_config)
    ...
    def create_sim_config(
        ...
        overwrite_config(
            ...
            config_to=agent_cfg,
            ignore_keys={
                "is_set_start_state", "sensors", "start_position", 
                "start_rotation", "mass", "linear_acceleration", 
                "angular_acceleration", "linear_friction", 
                "angular_friction", "coefficient_of_restitution"
            },
        )

        sensor_specifications = []
        for sensor in _sensor_suite.sensors.values():
            # Use CameraSensorSpec for RGB/Depth (supports HFOV directly)
            sim_sensor_cfg = habitat_sim.CameraSensorSpec()
            sim_sensor_cfg.uuid = sensor.uuid
            
            # Use habitat_sim.SensorType for mapping
            sensor_cast = cast(Any, sensor)
            sim_sensor_cfg.sensor_type = sensor_cast.sim_sensor_type
            
            # RESOLUTION: 0.3.3 expects [height, width]
            sim_sensor_cfg.resolution = [
                sensor.config.HEIGHT,
                sensor.config.WIDTH,
            ]

            # HFOV: Set the property directly instead of using .parameters
            # We use getattr to handle case sensitivity differences in Lab configs
        ...
        agent_cfg.action_space = registry.get_action_space_configuration(
            self.habitat_config.ACTION_SPACE_CONFIG
        )(self.habitat_config).get()
